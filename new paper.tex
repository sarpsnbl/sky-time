...
The estimation of time of day from a single outdoor photograph is a challenging problem at the meeting point of computer vision, digital image processing and environmental sensing. Modern mobile operating systems and cloud storage services attempt to semantically organize landscape photographs, yet their algorithms frequently produce unreliable temporal classifications due to an over-reliance on basic brightness metrics [1].

There are two main reasons for these mistakes that drive this research. First, nighttime images containing strong artificial light sources such as street lamps, stadium floodlights, or illuminated building facades are routinely misclassified as ``Daylight'' or ``Sunny'' because high localized luminance inflates the average pixel intensity and the bright spots trick the system into thinking the whole image is bright . Second, midday photographs captured under heavy cloud cover or in deeply shaded areas are often mislabeled as ``Night'' or ``Twilight'' due to globally suppressed light levels that mislead intensity-based classifiers meaning that the overall low light levels confuse classifiers that only look at intensity. 

These systematic errors underscore a fundamental limitation: average brightness alone is an insufficient proxy for solar position. A robust estimator must instead analyze complex visual cues including shadow geometry, atmospheric color temperature shifts from blue to warm orange, sky gradient profiles, and textural patterns in cloud formations to replicate the human cognitive ability to distinguish natural from artificial illumination [2].

A foundational study by Lalonde et al. [1] introduced the concept of utilizing ``weak cues'' extracted from various image segments which are the sky region, vertical surfaces, and the ground plane to estimate sun position and visibility. Their work demonstrated that while any single visual feature in isolation may be uninformative, the probabilistic integration of multiple complementary cues leads to a robust estimate of the sky dome's appearance. The present project adopts this multi-cue philosophy by analyzing both sky gradients and color temperatures to identify solar characteristics, extending the concept to a learned feature-fusion framework.
...
Both models converge consistently across folds. Swin-T's window-based attention appears to pick up on structured sky-region gradients more effectively in some folds, while ConvNeXt-Tiny's depthwise convolutions are more uniformly stable. Neither model suffers catastrophic divergence on any fold, indicating robust training under the shared augmentation and sampling regime.
